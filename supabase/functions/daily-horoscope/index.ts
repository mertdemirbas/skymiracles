...
var EnvKeys;
(function(EnvKeys) {
  EnvKeys["SUPABASE_URL"] = "SUPABASE_URL";
  EnvKeys["SUPABASE_KEY"] = "SUPABASE_SERVICE_ROLE_KEY";
  EnvKeys["OPENAI_KEY"] = "OPENAI_API_KEY";
})(EnvKeys || (EnvKeys = {}));
const supabase = createClient(Deno.env.get(EnvKeys.SUPABASE_URL), Deno.env.get(EnvKeys.SUPABASE_KEY));
const signs = [
  "aries",
  "taurus",
  "gemini",
...
async function fetchFromHoroscopeApp(sign) {
...
async function fetchFromBurcYorum(sign) {
  ...
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "You are a professional translator. Translate to natural Turkish."
        },
        {
          role: "user",
          content: text
...
serve(async ()=>{
  ...
  for (const sign of signs){
    ...
    let txt = await fetchFromHoroscopeApp(sign) || await fetchFromAztro(sign) || await fetchFromBurcYorum(sign) || "";
    if (!txt) {
    ...
    const { data, error } = await supabase.from("horoscopes").upsert({
      sign,
      date: today,
      text: txt
    }, {
